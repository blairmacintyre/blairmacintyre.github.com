...
module Jekyll
  ...
  class RenderFullWidthVarTag < Liquid::Tag
    ...
    def render(context)
      ...
      if @text[0].start_with?('http://', 'https://','//')
        "<figure class='fullwidth'><img src='#{@text[0]}'/>"+
        "<figcaption>#{context[@text[1].strip]}</figcaption></figure>"
      else
        "<figure class='fullwidth'><img src='#{baseurl}/#{@text[0]}'/>"+
        "<figcaption>#{context[@text[1].strip]}</figcaption></figure>"
      end
...
Liquid::Template.register_tag('fullwidthvar', Jekyll::RenderFullWidthVarTag)
